# Hyperworkflows adapter: Devin

Ports the harness-agnostic parts of Hyperworkflows to Devin (or Devin CLI). What ports cleanly:

| Layer | Portability | How |
|---|---|---|
| Methodology skills (oracle-forging, spec-attack, tricolor-reporting, adjudication-protocol, merge-discipline) | 100% | Installed as `.devin/skills/hyperworkflows-<name>/SKILL.md` |
...
This copies the five skills to `<project>/.devin/skills/hyperworkflows-*/` and generates
`<project>/.devin/hyperworkflows-role-prompts.md` from `agents/*.md` (frontmatter stripped,
...
2. Run verification through a subagent given ONLY the verifier contract from
   `hyperworkflows-role-prompts.md` (commands in, raw exit codes out).

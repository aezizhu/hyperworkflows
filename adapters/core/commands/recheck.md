Re-verify a past HW report with zero LLM judgment. Argument: a `runs/<run-id>` directory (default: the most recent under `runs/`).

1. Run: `node .hw/recheck.mjs <run-dir>` — it re-executes every recorded evidence command and diffs actual vs recorded exit codes.

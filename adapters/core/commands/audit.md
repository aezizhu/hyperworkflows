Run an Hyperworkflows evidence-grade audit of the given scope (default: files changed vs the default branch; else repo root). Follow `.hyperworkflows/rules.md` strictly.

1. `head=$(git rev-parse --short HEAD)`; `run_id=audit-<head>`; create `runs/<run_id>/verdicts/`.
2. Enumerate the scope TWO independent ways (filesystem walk via `git ls-files`, plus a symbol/build-graph pass). Reconcile with `node .hyperworkflows/adjudicate.mjs reconcile '<json>'`; resolve or explicitly report every disputed path. State the final denominator and method.
...
6. Execute all probes (acceptance + finding evidence), capture raw exit codes, and adjudicate ONLY via `node .hyperworkflows/adjudicate.mjs adjudicate '<json>'`. Write one canonical verdict file per unit under `runs/<run_id>/verdicts/`.

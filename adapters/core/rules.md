# Hyperworkflows operating rules (harness-agnostic core)

This project uses Hyperworkflows (Hyperworkflows) evidence discipline. These rules apply to any coding agent working here. The portable toolkit lives in `.hyperworkflows/` (Node >= 18, zero dependencies).
...
2. **Verdicts are computed by script, never by you.** You run commands and record raw exit codes; pass/fail comes from:
   `node .hyperworkflows/adjudicate.mjs adjudicate '{"probes":[{"cmd":"...","expect_exit":0}],"exit_codes":[{"cmd":"...","exit":0}]}'`
...
6. **Defend the denominator.** Enumerate scope at least two independent ways (e.g. `git ls-files` walk + symbol/build-graph pass); reconcile with `node .hyperworkflows/adjudicate.mjs reconcile '...'`; disputes are resolved explicitly or reported — never silently dropped.

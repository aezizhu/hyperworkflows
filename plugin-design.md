...
| W5 | Dynamic-workflow arg passing + module syntax | **VERIFIED — BOTH UNRELIABLE** (M2 runs 1+2, 2026-07-05: args destructured to `undefined` in two independent driver sessions; `export default` raises SyntaxError in the runtime, drivers must adapt engines to top-level scripts per run) | Engines self-resolve identity via a haiku probe (git HEAD + ACTIVE first line) when args are missing; signature defaults tolerate argless invocation; adaptation friction documented, outcome unaffected |

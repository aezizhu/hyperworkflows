...
| W2 | Workflow runtime module access: can workflow JS `import`/`require` (fs, path)? | **RESOLVED — inlining stands** (doctor run 2026-07-04: engines execute with inlined helpers; schema + models honored) | Adjudication helpers stay **inlined pure functions** synced by the bundler; standalone `.mjs` scripts serve hooks/commands |
| W3 | Does `agent()` inside a workflow accept `agentType` + `isolation: worktree`? | **VERIFIED — YES, with fully-qualified names** (doctor run on Claude Code 2.1.201: `agentType: "hyperworkflows:hyperworkflows-<role>"` binds the role's tool allowlist inside workflows; bare names error out; `isolation: worktree` honored — distinct toplevel confirmed) | Engines pass qualified `agentType` on every role call (17 bindings); embedded contracts remain as defense in depth |
...
| W5 | Dynamic-workflow arg passing + module syntax | **VERIFIED — BOTH UNRELIABLE, BOTH FIXED** (M2 runs 1+2, 2026-07-05: args destructured to `undefined` in two independent driver sessions; `export default` raises SyntaxError in the runtime — field-adapted `.run.js` scripts proved the working form: top-level body, `args` global, top-level `return`, `export const meta` kept) | Engines now SHIP runtime-native (converted from the field-proven adaptations, executed verbatim — no per-run rewriting); identity self-resolution via haiku probe (git HEAD + ACTIVE first line) covers missing args; `scripts/check-engines.mjs` (AsyncFunction compile) replaces `node --check` for engines in CI |

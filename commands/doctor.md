---
description: Troubleshooting - verify HW platform assumptions with evidence (never required for onboarding)
---

This is a diagnostic, NOT an onboarding step — HW works with zero setup. Run it when something misbehaves, or when the user wants evidence that everything is wired before a large fleet run.
...
2. **Engine availability (W1)**: confirm the plugin-shipped engines are reachable — the `hyperaudit`/`hyperapply`/`hw-sentinel` workflows registered by this plugin, with `${CLAUDE_PLUGIN_ROOT}/workflows/*.js` readable as the dynamic fallback. Note any project-local `.claude/workflows/` overrides and whether they diverge from the plugin versions.

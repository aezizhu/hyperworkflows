---
description: Re-verify a past Hyperworkflows report with zero LLM calls - re-runs every recorded evidence command and diffs exit codes
...
2. Report exactly what it prints, distilled:
   ...
   - Drift: list each drifted command with recorded vs actual exit code, and say plainly which report claims are now stale. Recommend re-running `/hyperworkflows:audit` scoped to the drifted units.

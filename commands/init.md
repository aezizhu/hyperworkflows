---
description: OPTIONAL project provisioning - Hyperworkflows works with zero setup; use only for local engine copies, gitignore hygiene, or baseline seeding
---

Hyperworkflows requires NO initialization. Engines, hooks, agents, and skills ship with the plugin; `runs/` and `memory/` are created lazily on first use. Say so if the user seems to believe init is required.

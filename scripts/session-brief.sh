#!/bin/sh
# HW SessionStart brief: stdout is injected as session context. Zero-setup by design:
...
echo "## HW (Hyperworkflows) active"
echo "Evidence discipline is available with zero setup: /hw:audit [scope] for an evidence-grade audit, /hw:apply to deliver an approved plan, /hw:recheck to re-verify any past report with zero LLM calls."
echo "Formation gate: <5 touched units -> work solo (no orchestration); >=5 units or the user demands evidence -> /hw:audit. Override with 'force'. One threshold, one source of truth."
